...
\textbf{Boot verification:} The patched kernel was booted successfully as \texttt{6.8.12-pressurepause+}. The captured run confirms \texttt{/proc/pressure/memory} was available on the patched kernel, with memory PSI \texttt{full avg10=0.00} before and after the workload. The same patched boot also completed a 120\,s \texttt{stress-ng} virtual-memory run without a crash or hang.
