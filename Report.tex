...
\textbf{Pending:} Screenshots of the GRUB menu (Advanced options $\rightarrow$ \texttt{6.8.12-baseline}) and \texttt{uname -r} output confirming \texttt{6.8.12-baseline} after reboot are not yet included. Boot verification should be completed before the final submission; the fallback Ubuntu entry remains available if the custom kernel fails.
...
Detailed BEFORE/AFTER flow diagrams and constant definitions (\texttt{PSI\_FULL\_THRESHOLD}, \texttt{PRESSURE\_PAUSE\_MAX\_MS}) will be completed with the Phase~3 patch.

\section{Implementation}

\textit{To be completed after applying the PressurePause patch to \texttt{mm/vmscan.c}, building \texttt{LOCALVERSION=-pressurepause}, and capturing \texttt{git format-patch} output in \texttt{kernel/patches/}.}

\section{Correctness and Safety}

\textit{To be completed. Planned invariants: bounded wait; never skip reclaim entirely; fall through on timeout or no reclaimable pages; GRUB fallback to Ubuntu kernel if the patched build panics.}
...
The baseline Linux~v6.8.12 kernel was built and installed with \texttt{LOCALVERSION=-baseline}. The reclaim component was identified on the direct allocation path from \texttt{page\_alloc.c} through \texttt{try\_to\_free\_pages} and \texttt{do\_try\_to\_free\_pages} in \texttt{vmscan.c}, with PSI providing the system-wide stall signal for the planned modification. Remaining work includes boot verification screenshots, patch implementation, patched-kernel benchmarks, and final results discussion.
