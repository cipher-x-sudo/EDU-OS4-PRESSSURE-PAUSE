...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item \texttt{try\_to\_free\_pages} --- \texttt{mm/vmscan.c} $\sim$6625
...
\textbf{Hook call site} (\texttt{mm/vmscan.c}):
...
PressurePause changes are applied in the vendored tree at \texttt{kernel/linux}. The same diff is exported as \texttt{kernel/patches/0001-pressurepause.patch}. Saved config: \texttt{kernel/configs/config-6.8.12-pressurepause} (\texttt{CONFIG\_PSI=y}, \texttt{LOCALVERSION="-pressurepause"}).
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item \texttt{mm/vmscan.c} --- threshold helpers, \texttt{pressure\_pause\_maybe()}, zone/kswapd helpers, debugfs counters, hook call at line~6397.
\end{itemize}
...
\begin{codebox}[mm/vmscan.c: threshold compare (lines 6189--6191, 6332--6336)]
\begin{lstlisting}[style=cstyle]
