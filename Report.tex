...
Matched pairs on the same host used \texttt{./bench-run.sh} with \texttt{stress-ng --vm N --vm-bytes P\% --timeout 120s}, \texttt{vmstat}, \texttt{pgmajfault}, and PSI samples. Representative directories: 8 workers / 93\% --- baseline \texttt{bench-6.8.12-baseline-20260519-135148} vs patched \texttt{bench-6.8.12-pressurepause+-20260519-145712}; 4 workers / 85\% --- baseline \texttt{bench-6.8.12-baseline-20260519-135638} vs patched average of \texttt{145355} and \texttt{145929}.
...
\midrule
8 vm / 93\% & Avg swap-out (KB/s) & 31222.5 & 19867.3 \\
8 vm / 93\% & \texttt{pgmajfault} $\Delta$ & 222 & 250 \\
4 vm / 85\% & Avg swap-out (KB/s) & 33380.0 & 20169.9 \\
4 vm / 85\% & Max swapped (KB) & 1215952 & 751542 \\
4 vm / 85\% & End swap (KB) & 91676 & 43742 \\
4 vm / 85\% & \texttt{pgmajfault} $\Delta$ & 5 & 215.5 \\
\bottomrule
\end{tabular}
\caption{Matched benchmark summary (May 2026). Patched runs used kernel built after the PSI basis-point fix.}
\end{table}

\subsection{Discussion}

After correcting the PSI gate (25\% \texttt{LOAD\_INT} bug $\rightarrow$ 0.01\% \texttt{some} avg10), PressurePause clearly reduced swap-out pressure and leftover swap in matched tests (roughly 36--52\% lower swap metrics on 4\,vm / 85\%). \texttt{stress-ng} completion time was unchanged within noise. The tradeoff is higher \texttt{pgmajfault} on patched runs: bounded pause defers synchronous reclaim, so more faults occur when pages are refaulted from swap. PSI \texttt{full} during stress remained low (peak $\sim$0.18\% in sampled runs), far below the obsolete 25\% gate; the active threshold is 0.01\% on \texttt{some}. Confirm coordination with \texttt{pressure-pause-debugfs-*.txt} (\texttt{activations} delta $> 0$) after installing the latest \texttt{pressurepause+} image.

\subsection{Limitations}

This is not a full statistical study. PSI samples are every 5\,s and can miss short spikes seen at the reclaim hook. Major-fault counts rise on the patched kernel. Install the rebuilt kernel and mount \texttt{debugfs} before trusting \texttt{activations} counters in \texttt{results/}.

\section{Conclusion}

The baseline Linux~v6.8.12 kernel and the PressurePause patch (\texttt{6.8.12-pressurepause+}) were built and compared under controlled \texttt{stress-ng} workloads. The modification is stable and, with the corrected PSI threshold, reduces swap pressure in matched benchmarks while increasing major page faults. The project demonstrates PSI-gated, bounded direct-reclaim coordination with measurable swap benefits and a documented fault-count tradeoff.
